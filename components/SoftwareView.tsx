...
import { Plus, Trash2, Edit2, Calendar, Users, X, History, IndianRupee, Layout, List, LayoutGrid, ChevronLeft, ChevronRight, Check } from 'lucide-react';
...
useEffect(() => {
  ...
  // Check if all filtered users are assigned
  const allFilteredUsersAssigned = filteredUsersForAssignment.length > 0 &&
    filteredUsersForAssignment.every(u => isUserAssigned(u.name));
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 h-full min-h-0">
                <div className={`${showLifecycle && editingItem ? 'lg:col-span-2' : 'lg:col-span-3'} flex flex-col h-full min-h-0 border-r`}>
                    ...
                    <form id="software-form" onSubmit={handleSubmit} className="space-y-6">
                      ...
                      <div className="pt-6 border-t">
                        ...
                        <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 mb-4">
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                            <div className="space-y-1">
                              <label className="text-[9px] font-bold text-slate-400 uppercase">1. Dept</label>
                              <select className="w-full border p-2 rounded-lg bg-white" value={selectedDeptForAssignment} onChange={e => setSelectedDeptForAssignment(e.target.value)}>
                                <option value="">All</option>
                                {departments.map(d => <option key={d.id} value={d.name}>{d.name}</option>)}
                              </select>
                            </div>
                            <div className="space-y-1 relative">
                              <label className="text-[9px] font-bold text-slate-400 uppercase">2. Employee(s)</label>
                              <div className="relative">
                                <button
                                  type="button"
                                  onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                                  className="w-full border p-2 rounded-lg bg-white text-left flex items-center justify-between"
                                >
                                  <span className="text-sm">
                                    {selectedUsersToAdd.length === 0 ? 'Select employees...' : `${selectedUsersToAdd.length} selected`}
                                  </span>
                                  ...
                                  <div className="absolute z-10 w-full mt-1 bg-white border rounded-lg shadow-lg max-h-64 overflow-y-auto">
                                    ...
                                    <div className="sticky top-0 bg-slate-50 border-b p-2 flex gap-2">
                                      <button
                                        ...
                                        className="flex-1 text-xs bg-blue-600 text-white px-2 py-1 rounded font-medium hover:bg-blue-700"
                                      ...
                                      <button
                                        ...
                                        className="flex-1 text-xs bg-slate-200 text-slate-700 px-2 py-1 rounded font-medium hover:bg-slate-300"
                                      ...
                                      return (
                                        <label
                                          ...
                                          className={`flex items-center gap-2 p-2 cursor-pointer hover:bg-slate-50 ${
                                            isAssigned ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : ''
                            ...
                            <div className="flex items-end gap-2">
                              ...
                              <button
                                ...
                                disabled={selectedUsersToAdd.length === 0}
                                className="bg-blue-600 text-white px-2 py-2 rounded-lg font-bold disabled:opacity-50 hover:bg-blue-700 transition-colors whitespace-nowrap"
                              >
                                Assign
                              </button>
                            </div>
                        ...
                        {(formData.assignedTo || []).length > 0 && (
                          <div className="mt-4">
                            <div className="text-xs font-bold text-slate-500 uppercase mb-2">Assigned Users ({formData.assignedTo?.length})</div>
                            <div className="bg-white border rounded-xl p-4 max-h-48 overflow-y-auto">
                                ...
                                {(formData.assignedTo || []).map((assignment, idx) => (
                                  <div key={idx} className="flex items-center justify-between p-2 bg-slate-50 rounded-lg border border-slate-200">
                                    <div className="flex-1">
                                      <div className="font-medium text-sm text-slate-800">{assignment.username}</div>
                                      <div className="text-xs text-slate-500">Assigned: {new Date(assignment.assignedDate).toLocaleDateString()}</div>
                                    </div>
                                    <button
                                      type="button"
                  ...
                  <div className="p-4 border-t bg-white flex justify-between items-center shrink-0">
                    <div>
                      <div className="text-[10px] font-black text-slate-400 uppercase">Asset Value</div>
                      <div className="text-xl font-black text-blue-600">₹{formTotalCost.toLocaleString('en-IN')}</div>
                    </div>
                    <div className="flex gap-3">
                      <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 font-bold text-slate-600">Cancel</button>
                      <button type="submit" form="software-form" className="px-8 py-2 bg-blue-600 text-white rounded-xl font-black shadow-lg shadow-blue-100 hover:bg-blue-700 transition-colors">Save License</button>
                    </div>
                  </div>
                </div>
