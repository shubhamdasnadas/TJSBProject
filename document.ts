...
const historyType = 2; // Use 2 for Logs to get severity
const body = {
  jsonrpc: "2.0",
  method: "history.get",
  params: {
...
export async function POST(req: Request) {
  try {
    ...
    const { itemid, time_from, time_till } = await req.json();
    ...
    /* =========================
       1️⃣ trigger.get (Zabbix 7)
       item → triggers (UI accurate)
    ========================= */
    const triggerBody = {
      jsonrpc: "2.0",
      method: "trigger.get",
    ...
    const triggers = triggerJson.result ?? [];
